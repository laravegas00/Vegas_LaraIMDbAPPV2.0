...
public class FavoritesSyncManager {
    ...
    public void removeFavoriteFromFirestore(String movieId) {
        getFavoritesCollection().document(movieId).delete()
                .addOnSuccessListener(aVoid -> Log.d(TAG, "Película eliminada de favoritos en Firestore"))
                .addOnFailureListener(e -> Log.e(TAG, "Error al eliminar película de favoritos en Firestore", e));
    }

    public void getUserFavorites(Consumer<List<Movie>> onSuccess, Consumer<Exception> onFailure) {
        CollectionReference favoritesRef = getFavoritesCollection();
        favoritesRef.get().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                List<Movie> userFavorites = new ArrayList<>();
                ...
                if (onSuccess != null) {
                    onSuccess.accept(userFavorites);
                }
            } else {
                Log.e(TAG, "Error al obtener las películas favoritas del usuario", task.getException());
                if (onFailure != null) {
